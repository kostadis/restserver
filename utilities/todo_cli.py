...
from todo_api_client.models.update_item import UpdateItem # Added for update command
...
def handle_update_item(args):
    ...
    if not any([args.name is not None, args.description is not None, args.priority is not None]):
        print("Error: No update fields provided. Please specify at least one field to update (--name, --description, --priority).")
        sys.exit(1)

    try:
        # 1. Fetch existing item to get current values and confirm existence
        existing_item = api.get_item_by_id(id=args.id)
        # print(f"Found item ID: {existing_item.id} (Name: {existing_item.name}, Description: {existing_item.description}, Priority: {existing_item.priority})")

        # 2. Determine new values, using existing if not provided by user
        updated_name = args.name if args.name is not None else existing_item.name
        updated_priority = args.priority if args.priority is not None else existing_item.priority

        # Handle description: if --description is given (even if empty string), use it. Otherwise, use existing.
        if args.description is not None:
            updated_description = args.description
        else:
            # Use existing_item's description if it has one, otherwise it remains None (which is valid for Optional[str])
            updated_description = existing_item.description if hasattr(existing_item, 'description') else None

        # 3. Construct UpdateItem payload
        # UpdateItem model requires 'name' and 'priority'.
        update_payload = UpdateItem(
            name=updated_name,
            priority=updated_priority,
            description=updated_description  # This can be None if not provided and not existing, or if explicitly set to None via API
        )

        # 4. Call API to update
        updated_item_response = api.update_item_by_id(id=args.id, update_item=update_payload)

        print("Successfully updated item:")
        print(f"  ID: {updated_item_response.id}")
        print(f"  Name: {updated_item_response.name}")
        # Handle optional description for printing
        description_to_print = 'N/A'
        if hasattr(updated_item_response, 'description') and updated_item_response.description is not None:
            description_to_print = updated_item_response.description
        print(f"  Description: {description_to_print}")
        print(f"  Priority: {updated_item_response.priority}")

    except NotFoundException:
        print(f"Error: Item with ID {args.id} not found. Cannot update.")
        sys.exit(1)
    except BadRequestException as e:
        print(f"Error updating item (Invalid Request for ID {args.id}):")
        _print_api_exception_details(e)
        sys.exit(1)
    except ApiException as e:
        print(f"API Error updating item {args.id}:")
        _print_api_exception_details(e)
        sys.exit(1)
    except Exception as e:
        print(f"An unexpected error occurred while updating item {args.id}: {e}")
        sys.exit(1)
...
def main():
    ...
    update_parser.add_argument("--description", type=str, help="New description for the item (provide an empty string \"\" to clear it, if API supports)")
